package model

import (
	"github.com/jinzhu/gorm"
	"golang.org/x/crypto/bcrypt"
)

// Admin 用户模型
type Admin struct {
	gorm.Model
	UserName       string
	PasswordDigest string
	Status         string
}

// GetAdmin 用ID获取管理员
func GetAdmin(ID interface{}) (Admin, error) {
	var admin Admin
	result := DB.First(&admin, ID)
	return admin, result.Error
}

// AdminSetPassword 设置密码
func (admin *Admin) AdminSetPassword(password string) error {
	bytes, err := bcrypt.GenerateFromPassword([]byte(password), PassWordCost)
	if err != nil {
		return err
	}
	admin.PasswordDigest = string(bytes)
	return nil
}

// AdminCheckPassword 校验密码
func (admin *Admin) AdminCheckPassword(password string) bool {
	err := bcrypt.CompareHashAndPassword([]byte(admin.PasswordDigest), []byte(password))
	return err == nil
}
